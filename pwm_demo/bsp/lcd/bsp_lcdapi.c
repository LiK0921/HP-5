...
void lcd_drawline(unsigned short x1, unsigned short y1, unsigned short x2, unsigned short y2)
{
	...
	{  
		...
		lcd_drawpoint(uRow, uCol, tft_lcd_struct.forecolor);/* 画点 */
		xerr += delta_x ; 
		yerr += delta_y ; 
		if(xerr > distance) 
		{ 
...
void lcd_draw_Circle(unsigned short x0,unsigned short y0,unsigned char r)
{
    ...
    {
        lcd_drawpoint(x  + mx, y  + my, tft_lcd_struct.forecolor);
        lcd_drawpoint(y  + mx, x  + my, tft_lcd_struct.forecolor);
        lcd_drawpoint(-x + mx, y  + my, tft_lcd_struct.forecolor);
        lcd_drawpoint(-y + mx, x  + my, tft_lcd_struct.forecolor);
...
{  							  
	...
	{   
		...
		for(t1 = 0; t1 < 8; t1++)
		{			    
			if(temp & 0x80)lcd_drawpoint(x, y, tft_lcd_struct.forecolor);
			else if(mode==0)lcd_drawpoint(x, y, tft_lcd_struct.backcolor);
			temp <<= 1;
			y++;
			if(y >= tft_lcd_struct.height) return;			/* 超区域了 */	
			if((y - y0) == size)
			{
				y = y0;
				x++;
				if(x >= tft_lcd_struct.width) return;		/* 超区域了 */
				break;
			}
		}  	 
	}  		    	   	 	  
...
void integer_display(unsigned short x, unsigned short y, unsigned char size, signed int num)
{
	...
	lcd_fill(x, y, x + 50, y + size, tft_lcd_struct.backcolor);
...
void decimals_display(unsigned short x, unsigned short y, unsigned char size, signed int num)
{
	...
	if(num < 0)
		...
		sprintf(buf, "%d.%d", integ, fract);
	lcd_fill(x, y, x + 60, y + size, tft_lcd_struct.backcolor);
...
void lcd_draw_bigpoint(unsigned short x, unsigned short y, unsigned int color)
{
	...
	if(x < tft_lcd_struct.width -1) {
		lcd_drawpoint(x + 1, y, color);
	}

	if(y < tft_lcd_struct.height -1) {
		lcd_drawpoint(x , y + 1, color);
	}

	if((x < tft_lcd_struct.width -1) && (y < tft_lcd_struct.height -1)) {
