...
/* 
 * 外设指针 :
 * 将外设基地址转成相应外设结构体类型的指针，所以指针访问成员变量的时候，那个成员变量在内存中的地址就刚好是外设该寄存器所在
 * 			所以往该成员变量写数据，就是往那个地址单元写数据。
 * 本质：该结构体不存在，就是存在一个该类型的指针而已，指针中的某个成员变量，就是基地址的偏移多少的单元，往指针的成员写数据，
 * 		就是往外设基地址的偏移多少地址单元写入数据
 */
